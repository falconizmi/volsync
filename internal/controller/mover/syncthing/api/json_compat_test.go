...
var _ = Describe("Operator config modification flow", func() {
	...
	It("full operator first-reconcile preserves all critical config", func() {
		...
		// Step 2: ShareFoldersWithDevices
		st.ShareFoldersWithDevices()
	...
	It("preserves top-level options section through roundtrip", func() {
		// The config template disables discovery and relays via the options
		// section. Our Configuration struct doesn't model options, but the
		// raw JSON preservation must keep it. Without this, PUT would reset
		// globalAnnounceEnabled to true (syncthing's default).
		...
		var cfg config.Configuration
		err := json.Unmarshal([]byte(configWithOptions), &cfg)
		Expect(err).NotTo(HaveOccurred())
		...
		// Options section must survive
		Expect(raw).To(HaveKey("options"), "options section must survive roundtrip")

		var options map[string]json.RawMessage
		err = json.Unmarshal(raw["options"], &options)
		Expect(err).NotTo(HaveOccurred())

		var globalAnnounce bool
		err = json.Unmarshal(options["globalAnnounceEnabled"], &globalAnnounce)
		Expect(err).NotTo(HaveOccurred())
		Expect(globalAnnounce).To(BeFalse(),
			"globalAnnounceEnabled must stay false, not reset to syncthing default true")

		var relays bool
		err = json.Unmarshal(options["relaysEnabled"], &relays)
		Expect(err).NotTo(HaveOccurred())
		Expect(relays).To(BeFalse())

		// LDAP section also preserved
		Expect(raw).To(HaveKey("ldap"))
	})
})
